-- Chic Estilistes — esquema completo (agrupa 0001_init.sql + 0002_reminders.sql
-- + 0003_lang_support.sql en un único archivo).
--
-- Pensado para un proyecto de Supabase nuevo: ejecuta esto entero una sola
-- vez en SQL Editor y ya tienes web + panel + WhatsApp (reservas,
-- recordatorios y multi-idioma) listos.
--
-- Si tu proyecto YA tenía aplicado alguno de los tres archivos por
-- separado, también puedes ejecutar este igualmente sin miedo: todo usa
-- "if not exists" / "on conflict do nothing", así que lo que ya existía
-- se deja tal cual y solo se crea lo que faltaba.

create extension if not exists "pgcrypto";

-- ================================================================
-- 1) Esquema base (antes: 0001_init.sql)
-- ================================================================
...
drop policy if exists "public read stylists" on stylists;
...
drop policy if exists "public insert blocked_slots" on blocked_slots;
...
-- La usan por igual la web, el simulador de WhatsApp y el webhook real de
-- WhatsApp, para que los tres canales vean siempre la misma disponibilidad.
...
-- ================================================================
-- 2) Recordatorios por WhatsApp (antes: 0002_reminders.sql)
-- ================================================================
...
-- Programar el envío automático: la función whatsapp-reminders no se
-- ejecuta sola. Forma más simple, sin SQL: Dashboard > Database > Cron
-- Jobs > "Create job" > tipo "Edge Function" > whatsapp-reminders > cada
-- hora. Equivalente por SQL/CLI (sustituye TU-PROYECTO y TU_SERVICE_ROLE_KEY):
--
...
--   );
--
-- Para revisar que se ejecuta:
--   select * from cron.job_run_details order by start_time desc limit 5;

-- ================================================================
-- 3) Conversación de WhatsApp + multi-idioma (antes: 0003_lang_support.sql)
-- ================================================================
...
-- ================================================================
-- 4) Varios huecos por estilista en la página de reservas (antes: 0004_multi_slots.sql)
-- ================================================================
